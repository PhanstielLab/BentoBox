...
bb_annoDomains <- function(plot, data, half = "inherit",
    ...
    domainAnnotation <- function(df, object, half) {
        if (half == "bottom") {
            x0_1 <- as.numeric(df[2])
            x1_1 <- as.numeric(df[3])
            y_1 <- as.numeric(df[2])

            x_2 <- as.numeric(df[3])
            y0_2 <- as.numeric(df[2])
            y1_2 <- as.numeric(df[3])
            ...
            x_1 <- as.numeric(df[2])
            y0_1 <- as.numeric(df[2])
            y1_1 <- as.numeric(df[3])

            x0_2 <- as.numeric(df[2])
            x1_2 <- as.numeric(df[3])
            y_2 <- as.numeric(df[3])
    ...
    bed_subset <- bed[which(bed[, 1] == bb_domains$chrom & bed[, 2] <=
        bb_domains$chromend & bed[, 3] >=
